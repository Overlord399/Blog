...
use \Exception;
use \PDO;
use AppBundle\NewsSite\Comment as ChildComment;
use AppBundle\NewsSite\CommentQuery as ChildCommentQuery;
use AppBundle\NewsSite\Item as ChildItem;
use AppBundle\NewsSite\ItemQuery as ChildItemQuery;
use AppBundle\NewsSite\User as ChildUser;
use AppBundle\NewsSite\UserQuery as ChildUserQuery;
use AppBundle\NewsSite\Map\CommentTableMap;
 ...
 *
 *
 * @package    propel.generator.src\AppBundle.NewsSite.Base
 */
abstract class Item implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\AppBundle\\NewsSite\\Map\\ItemTableMap';
    ...
    /**
     * Initializes internal state of AppBundle\NewsSite\Base\Item object.
     */
     ...
     * @return $this|\AppBundle\NewsSite\Item The current object (for fluent API support)
     */
    ...
    public function setDate($v)
    {
        ...
        if ($this->date !== null || $dt !== null) {
            if ($this->date === null || $dt === null || $dt->format("Y-m-d") !== $this->date->format("Y-m-d")) {
                ...
                $this->modifiedColumns[ItemTableMap::COL_DATE] = true;
            }
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[ItemTableMap::COL_USER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ItemTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->date = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ItemTableMap::translateFieldName('UserId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\AppBundle\\NewsSite\\Item'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commentsScheduledForDeletion !== null) {
                if (!$this->commentsScheduledForDeletion->isEmpty()) {
                    \AppBundle\NewsSite\CommentQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ItemTableMap::COL_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'date';
        }
        if ($this->isColumnModified(ItemTableMap::COL_USER_ID)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AppBundle\NewsSite\Item
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AppBundle\NewsSite\Item The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ItemTableMap::COL_DATE)) {
            $criteria->add(ItemTableMap::COL_DATE, $this->date);
        }
        if ($this->isColumnModified(ItemTableMap::COL_USER_ID)) {
            $criteria->add(ItemTableMap::COL_USER_ID, $this->user_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AppBundle\NewsSite\Item (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AppBundle\NewsSite\Item Clone of current object.
     * @throws PropelException
     */
    ...
    public function initComments($overrideExisting = true)
    {
        ...
        $this->collComments->setModel('\AppBundle\NewsSite\Comment');
    }
